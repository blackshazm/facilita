...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMessage(error.message || 'Erro ao entrar. Verifique suas credenciais.')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-muted/20 px-4">
      <Link href="/" className="absolute top-8 left-8 flex items-center gap-2">
        ...
        <span className="text-xl font-bold tracking-tight">Chama<span className="text-primary text-primary">Ai</span></span>
      </Link>
      ...
      <Card className="w-full max-w-md border-none shadow-2xl glass">
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center px-1">
                ...
                <Link href="#" className="text-xs text-primary font-semibold hover:underline">Esqueceu a senha?</Link>
              </div>
              <Input 
                type="password" 
                placeholder="••••••••" 
            ...
            <Button type="submit" disabled={isLoading} className="w-full h-14 rounded-2xl bg-primary text-lg font-bold shadow-xl shadow-primary/20 hover:shadow-primary/30 transition-all active:scale-[0.98]">
              {isLoading ? <Loader2 className="h-6 w-6 animate-spin" /> : 'Entrar na Conta'}
            </Button>
          </form>
        </CardContent>
        <CardFooter className="flex flex-col gap-4 border-t pt-6 bg-muted/30">
          <p className="text-sm text-muted-foreground">
            Ainda não tem conta?{' '}
            <Link href="/register" className="text-primary font-bold hover:underline">Crie agora</Link>
          </p>
          <div className="w-full h-px bg-muted flex items-center justify-center">
            <span className="bg-muted px-4 text-[10px] uppercase font-black text-muted-foreground tracking-widest">Ou use</span>
          </div>

          <Button type="button" onClick={handleGoogleLogin} variant="outline" className="w-full h-12 rounded-xl border-2 font-bold flex gap-3 hover:bg-white transition-all">
